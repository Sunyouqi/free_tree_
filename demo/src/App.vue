      ...
      <div class="container">
        <h1>
          ...
          <VueToggles
            ...
            dot-color="#F3F1FA"
            :dot-size="20"
            unchecked-text-color="black"
            unchecked-text="default"
            checked-text="white"
            font-weight="bold"
            checked-bg="#122235"
            unchecked-bg="#9FFFFA"
            @click="onToggleTheme(isChecked)"
          />
        </h1>
...
const selectedTheme = ref<'' | 'white-theme'>(themes[1].input)
const isOnMobile = ref(false)

function onToggleTheme(isChecked: boolean) {
  if (isChecked == false) {
    selectedTheme.value = themes[1].input
  } else {
    selectedTheme.value = themes[0].input
  }
